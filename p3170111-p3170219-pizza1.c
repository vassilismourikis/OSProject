#include "p3170111-p3170219-pizza1.h"
...
int main(int argc,char **argv)
{
	...
	//checing oparators
	if(argc<3){
	printf("Not expected values given\n");
	exit(-1);
	}
	//init args
	...
	//array with structs of all orders
	ords=malloc(sizeof(struct order)*cust);
	times=malloc(sizeof(int)*cust);
	t=malloc(sizeof(struct timespec)*cust);
...
//CHECK
	...
	printf("Max time: %d\n",maxTime);
	//freeing allocated with malloc space 
	free(ids);
	free(times);
	free(ords);
	free(t);
...
void * prep(void * ar){
	...
	return_code=clock_gettime(CLOCK_REALTIME,&t[preperator->number]);
//CHECK
 	if(return_code==-1){
...
//CHECK
//THREAD'S STARTING TIME
	times[*num]=t[preperator->number].tv_sec;
	//START PREPERATORS
	//lock
...
//CHECK
	times[*num]=t[preperator->number].tv_sec-times[*num];
//LOCKING SCREEN
